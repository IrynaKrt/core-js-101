...
function findFirstSingleChar(str) {
  const char = str.split('').find((el) => str.indexOf(el) === str.lastIndexOf(el));
...
function isCreditCardNumber(ccn) {
  ...
  if (!((str.length - 1) % 2)) {
    for (let i = 1; i < str.length; i += 2) {
      const el = +(str[i] * 2 > 9 ? str[i] * 2 - 9 : str[i] * 2);
      res += +str[i - 1] + el;
    }
...
function getDigitalRoot(num) {
  return num < 10 ? num : getDigitalRoot(num.toString().split('').reduce((sum, el) => sum + +el, 0));
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const indexO = open.indexOf(arr[i]);
    const indexC = close.indexOf(arr[i]);
    if (indexC !== -1 && emptyArr.length === 0) return false;
    if (indexC !== -1 && indexC !== emptyArr.pop()) return false;
    if (indexO !== -1) emptyArr.push(indexO);
  }
  return str === '' || !emptyArr.length;
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    let a = '';
    for (let j = 0; j < 3; j += 1) {
      if (a === '' && position[j][i] !== undefined) {
        a = position[j][i];
      } else if (a[0] === position[j][i]) {
        a += position[j][i];
      } else {
        break;
      }
